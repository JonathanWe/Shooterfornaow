        ...
        public void Draw()
        {
            ...
            if (Acceleration.X != 0)
            {
                if (lookRight)
                {
                    ...
                    Engine.Camera.Draw(sheet.Texture, new Vector2(Position.X + headR1Offset.X, (Position.Y + headR1Offset.Y)), new Vector2(headR1Size.X, headR1Size.Y), sheet.GetSprite(CharacterColor + "HeadR1"), color, 0.2f, Vector2.Zero, 0);
                    Engine.Camera.Draw(sheet.Texture, new Vector2(Position.X + bodyRunROffset.X, Position.Y + bodyRunROffset.Y), new Vector2(bodyRunRSize.X, bodyRunRSize.Y), runFrame, color, 0.11f, Vector2.Zero, 0);
                }
                else
                {
                    var runFrame = runAnimationL.GetFrame();
                    Engine.Camera.Draw(sheet.Texture, new Vector2(Position.X + headL1Offset.X, Position.Y + headL1Offset.Y), new Vector2(headL1Size.X, headL1Size.Y), sheet.GetSprite(CharacterColor + "HeadL1"), color, 0.2f, Vector2.Zero, 0);
                    Engine.Camera.Draw(sheet.Texture, new Vector2(Position.X + bodyRunLOffset.X, Position.Y + bodyRunLOffset.Y), new Vector2(bodyRunLSize.X, bodyRunLSize.Y), runFrame, color, 0.11f, Vector2.Zero, 0);
                }
            }
            else
            {
