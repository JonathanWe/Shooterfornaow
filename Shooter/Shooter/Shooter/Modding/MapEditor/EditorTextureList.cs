...
namespace Shooter.Modding.MapEditor
{
    public class EditorTextureList
    {
        ...
        public void Draw() 
        {
            Engine.SpriteBatch.Draw(GameResources.GUISheet.Texture, new RectangleF(Vector2.Zero, Size), GameResources.GUISheet.GetSprite("Y"), Color.White, 0, Vector2.Zero, SpriteEffects.None, 0.52f);
            Engine.SpriteBatch.Draw(GameResources.GUISheet.Texture, new RectangleF(new Vector2(0, Engine.WindowHeight - BottomSize.Y), BottomSize), GameResources.GUISheet.GetSprite("X"), Color.White, 0, Vector2.Zero, SpriteEffects.None, 0.5201f);
            for (int i = 0; i < Grid.Textures.Count; i++)
            {
                Vector2 position = getPositionFromindex(i);
                if (i == SelectedIndex)
                    Engine.SpriteBatch.Draw(GameResources.WhiteTexture, new RectangleF(position - new Vector2(textureDistance / 2, textureDistance / 2), textureSize + new Vector2(textureDistance, textureDistance)), null, new Color(1f, 1f, 0f, 0.00f), 0, Vector2.Zero, SpriteEffects.None, 0.521f);
                else if (Engine.Collide(position, textureSize, Engine.MousePosition, Vector2.Zero))
                    Engine.SpriteBatch.Draw(GameResources.WhiteTexture, new RectangleF(position - new Vector2(textureDistance / 2, textureDistance / 2), textureSize + new Vector2(textureDistance, textureDistance)), null, Color.CornflowerBlue, 0, Vector2.Zero, SpriteEffects.None, 0.521f);
